...
def test_package_reports_include_repository_attribution() -> None:
    ...
    assert "⭐ Star the project" in markdown
    assert "⭐ Star the project" in html
    assert "https://github.com/kannandreams/secchi" in markdown
    assert "https://github.com/kannandreams/secchi" in html
    assert "https://github.com/example/demo" in markdown
...
def test_project_report_contains_sources_in_all_formats() -> None:
    ...
    assert (
        ...
        in render_project_report("md", report)
    )
    assert "1.2.3" in render_project_report("html", report)
    assert (
        'Generated by <a href="https://github.com/kannandreams/secchi">Secchi</a>'
        in render_project_report("html", report)
    )
...
def test_search_service_prioritizes_exact_matches_and_survives_registry_errors(
    ...
    results = asyncio.run(
        PackageSearchService().search(
            "duckdb", registries=[Registry.PYPI, Registry.NPM]
        )
    )
