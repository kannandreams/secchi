...
def build_project_report(
    ...
    for ref in project.packages:
        ...
        sources.append(ProjectSourceReport(ref, result.info, result.derived, error, warnings))
...
def _source_markdown_row(source: dict) -> str:
    ...
    return (
        ...
        f"{downloads} | {_source_status(source)} |"
    )
...
def _markdown_attribution(repository_url: str | None) -> str:
    ...
    return f"Generated by Secchi{star}"
...
def _markdown_warnings(warnings: list[object] | None) -> str:
    if not warnings:
        return ""
    rows = "\n".join(
        f"- `{warning.source}`: {warning.message}" for warning in warnings
    )
...
def _html_attribution(repository_url: str | None) -> str:
    ...
    return f'<footer style="margin-top:2rem;color:#5f6b7a">Generated by Secchi{star}</footer>'
